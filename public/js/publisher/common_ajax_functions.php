...
function validate_subscription_form(pFormId,ajax_url)
{
	if(pFormId)
	{
		...
		{
			$('#err_chkterms').html('Please check if you agree terms and conditions');
			return false;
		}
...
function inactivesubscription(ajax_url,tab_ajax)
{

    if(confirm('Are you sure want to inactivate this subscription?'))
	{
...
function activesubscription(ajax_url,tab_ajax)
{

    if(confirm('Are you sure want to activate this subscription?'))
	{
    var priceoutput =$("#priceoutput");
	var refreshrate =$("#refreshrate");
	var prid = $("#productid").val();
...
function updatesubscription2(priceid,ajaxurl,productid)
{ 
	if($('#country_sub').val()=='')
		{
			$('#err_country').html('Please select country for subscription');
	...
	$.ajax({
		...
		success: function(result) {
			...
			if(pub_type_id == '3')
			{
				$("#title_text").show();
				$("#reqird").hide();
				$("#showsubscription").hide();
				$("#refreshrate").hide();
				//alert(pub_type_id);
			}
			else if(pub_type_id=='1' || pub_type_id=='2')
			{
