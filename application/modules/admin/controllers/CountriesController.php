	...
	public function addAction()
    {
		...
		if($this->getRequest()->isPost())
		{
			...
			if(count($formErrors)==0)
			{
				...
				if($result>0)
				{
					$this->_flashMessenger->addMessage('<div class="div-success">Country added successfully</div>');
					$this->_redirect('admin/countries/');
				}
				else
				{
					$this->view->errorMessage1='<div class="div-error">Sorry!, unable to add country</div>';
				}
			}
			else
			{
		...
		if($id>0 && $this->modelCountries->isExist($id))
		{
			...
			if($this->getRequest()->isPost())
			{
				...
				if(count($formErrors)==0)
				{
					...
					$this->_flashMessenger->addMessage('<div class="div-success">Country updated successfully</div>');
					$this->_redirect('admin/countries/');
				}
				else
				{
