...
class Admin_StoresController extends Zend_Controller_Action
{
	...
	public function addAction()
    {
		...
		$ThumbSquareSize 		= 60; //Thumbnail will be 100x100
		...
		{
			if(!$this->modelBooks->cropImage($CurWidth,$CurHeight,$ThumbSquareSize,$thumb_DestRandImageName,$CreatedImage,$Quality,$ImageType,90,60))
			{
				$this->_flashMessenger->addMessage('<div class="div-error">Error Creating thumbnail for sliders!</div>');
			}
			if(!$this->modelBooks->cropImage($CurWidth,$CurHeight,$ThumbSquareSize,$thumb1_DestRandImageName,$CreatedImage,$Quality,$ImageType,90,60))
			{
			...
			list($ResizedWidth,$ResizedHeight)=getimagesize($DestRandImageName);
				...
				$formData['is_store']=1;
				$formData['country_flag']=	$ThumbPrefix.$NewImageName;		
				...
				*/
				...
				if($_FILES['ImageFile']['tmp_name']!='')
				{
					if(!isset($_FILES['ImageFile']) || !is_uploaded_file($_FILES['ImageFile']['tmp_name']))
					{
						...
						$formDataFlag['country_flag'] = $ThumbPrefix.$NewImageName;	
						$this->modelCountries->update($formDataFlag,'id='.$id);
	...
	public function deleteflagAction()
	{
		...
		{
			$countryInfo=$this->modelCountries->fetchRow('id='.$id);
			//echo $_SERVER['DOCUMENT_ROOT'].$countryInfo['country_flag'];		
			unlink(USER_UPLOAD_DIR.$countryInfo['country_flag']);
			$formData['country_flag'] = '';
			$this->_flashMessenger->addMessage('<div class="div-success">Image deleted successfully</div>');
			$result=$this->modelCountries->update($formData,'id='.$id);
			$this->_redirect('admin/stores/edit/id/'.$id);
		}
	...
	{
		$this->view->messages = $this->_flashMessenger->getMessages();
		$this->modelCurrency = new Admin_Model_DbTable_Currency();
		$currencyList = $this->modelCurrency->fetchAll();
	...
	public function addcurrencyAction()
	{
		...
		{
			$formData=$this->getRequest()->getPost();
			if(!(isset($formData['currency_name'])) || trim($formData['currency_name'])=="")
			{
				$formErrors['currency_name']='Please enter Currency';
			}
			if(count($formErrors)==0)
			{
				if($formData['currency_name']!='')
				{
					$currency_val = explode("~~",$formData['currency_name']);	
					$currency_name = $currency_val[0];
					$currency_sign = $currency_val[1];	
					$data_val = array("currency_name"=>$currency_name,"currency_sign"=>$currency_sign);	
					$fetch_val = $this->modelCurrency->fetchRow("currency_name ='".$currency_name."'");
					if(count($fetch_val)==0)
					{
						$result=$this->modelCurrency->insert($data_val);
					}
					else {
						$this->_flashMessenger->addMessage('<div class="div-success">Already exists.</div>');
					}
				}
				if($result>0)
				{
					$this->_flashMessenger->addMessage('<div class="div-success">Currency added successfully</div>');
					$this->_redirect('admin/stores/listcurrency/');
				}
				else
				{
					$this->view->errorMessage='<div class="div-error">Sorry!, unable to add continent</div>';
	...
	{				
	  ...
	  //exit ();
		if($this->getRequest()->isPost())
		{
			...
			if(!(isset($formData['currency_name'])) || trim($formData['currency_name'])=="")
			{
				$formErrors['currency_name']='Please enter Currency Name';
			}
			if(count($formErrors)==0)
			{
				if($formData['currency_name']!='')
				{
					...
					$currency_sign = $currency_val[1];	
					$data_val = array("currency_name"=>$currency_name,"currency_sign"=>$currency_sign);	
					$result=$this->modelCurrency->update($data_val,'currency_id='.$id);					
					$this->_flashMessenger->addMessage('<div class="div-success">Currency updated successfully</div>');
					$this->_redirect('admin/stores/listcurrency/');
				}
			}
			else
		...
		if($id!='')
		{
			$currencyInfo=$this->modelCurrency->fetchRow('currency_id='.$id);
			$formData=$currencyInfo->toArray();
