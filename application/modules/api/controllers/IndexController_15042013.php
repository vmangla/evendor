		...
		if(is_array($formData))
		{
           	...
           	switch($jsonObj->apicall)
			{
				...
				case "UserRegistration":
					...
					else 
					{
							...
							else
							{
								...
								$sql = "INSERT INTO pclive_companies (parent_id,group_id,first_name,last_name,account_type,user_name,user_email,user_password,country,status,added_date,updated_date)VALUES('0','0','".$jsonObj->FirstName."','".$jsonObj->LastName."','2','$user_name','".$jsonObj->EmailId."','".$jsonObj->Password."','".$jsonObj->Country."','0', NOW(),NOW())";
				...
				case "UserLogin":
					...
					{
						$sql = 'SELECT * FROM pclive_companies where user_email="'.$jsonObj->EmailId.'" AND user_password="'.$jsonObj->Password.'" AND parent_id="0" AND account_type<>"3"';
						$result = $db->query($sql);
						$record = $result->FetchAll();		
				...
				case "GetLibrary":
					...
					{
						$sql="SELECT prod.*,c.category_name, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id=storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id WHERE prod.admin_approve='1' AND storeprice.country_id='$jsonObj->StoreId' ORDER BY prod.cat_id ASC";
					}
					else
					{
						$sql="SELECT prod.*,c.category_name, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id = storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id WHERE prod.admin_approve='1' ORDER BY prod.cat_id ASC";
					}
				...
				case "UserUpdate":
					...
					{	
						$Errorresponse="";
						if(!(isset($jsonObj->FirstName)) || trim($jsonObj->FirstName)=="" || !(isset($jsonObj->LastName)) || trim($jsonObj->LastName)=="" || !(isset($jsonObj->Country)) || trim($jsonObj->Country)=="" || !(isset($jsonObj->Phone)) || trim($jsonObj->Phone)=="")
						{
							$Errorresponse='[{"ParameterMissing":{';
							if(!(isset($jsonObj->FirstName)) || trim($jsonObj->FirstName)=="") 
							{
							...
							if(!(isset($jsonObj->Phone)) || trim($jsonObj->Phone)=="") 
							{
								$Errorresponse.='"Password":"Please Enter Phone",';
							}
						...
						{
					    	$sql = "UPDATE pclive_companies SET first_name='$jsonObj->FirstName', last_name='$jsonObj->LastName', country='$jsonObj->Country', phone='$jsonObj->Phone' where id='$jsonObj->id'";
							$result = $db->query($sql);
														
							if($result)
							{
								  $response = '[{"UpdationResponse":{
