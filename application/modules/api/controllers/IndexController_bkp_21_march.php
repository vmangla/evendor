           	...
           	switch($jsonObj->apicall)
			{
				...
				case "UserLogin":
				    if(isset($jsonObj->EmailId) && !empty($jsonObj->EmailId) && isset($jsonObj->Password)) 
					{
						...
						$sql2 = 'SELECT * FROM pclive_users where emailid="'.$jsonObj->EmailId.'" AND password="'.md5($jsonObj->Password).'"';
						...
						$record1 = $result1->FetchAll();	
						  ...
						  {
							   $devicesqlinsert = 'insert into pclive_deviceID set user_publisher_id="'.$record[0]['id'].'", table_name="pclive_companies",device_id="'.$jsonObj->DeviceId.'"';
						  ...
						  //can not login with this device id	
						    ...
						    $response = '{
							"Message":"Can not login more than 3 devices",
							"apikey":"",
							"error":"true"
							}';
						...
						else if(count($record2)>0 && $record2[0]['user_type']!='author')
						{
						...
						$sql1 = "SELECT * FROM pclive_country where id='".$record2[0]['country']."' ";
						$result1 = $db->query($sql1);
						$record1 = $result1->FetchAll();	
							...
							"apikey":"'.$apikeyrecord[0]['apikey'].'",
							"userid":"'.$record2[0]['id'].'",
							"UserType":"'.$record2[0]['user_type'].'",
							"countryid":"'.$record2[0]['country'].'",
							"Country" : "'.$record1[0]['country'].'", 
							"FirstName" : "'.$record2[0]['first_name'].'",
							"LastName"	: "'.$record2[0]['last_name'].'","Username":"'.$record2[0]['emailid'].'","Userpassword":"'.$record2[0]['password'].'","error":"false"
							}';
						  //have to check fo device id
						  //if count<3 then onsert
						  
						  $devicesql = 'SELECT * FROM  pclive_deviceID where user_publisher_id="'.$record2[0]['id'].'" and table_name="pclive_users"';
						  $deviceresult = $db->query($devicesql);
						  $devicerecord = $deviceresult->FetchAll();
						  ...
						  $devicesqln = 'SELECT * FROM  pclive_deviceID where user_publisher_id="'.$record2[0]['id'].'" and table_name="pclive_users" and device_id="'.$jsonObj->DeviceId.'"';
						  $deviceresultn = $db->query($devicesqln);
						  $devicerecordn = $deviceresultn->FetchAll();
						  ...
						  if (count($devicerecord)<3)
						  {
							   $devicesqlinsert = 'insert into pclive_deviceID set user_publisher_id="'.$record2[0]['id'].'", table_name="pclive_users",device_id="'.$jsonObj->DeviceId.'"';
						       $deviceinsertresult = $db->query($devicesqlinsert);
							//insert  
						  }
						  else
				...
				case "UserDetailIphone":
				    if(!empty($jsonObj->id) && $jsonObj->id>0) 
					{
						...
						$record1 = $result1->FetchAll();	
							...
							"countryid" : "'.$record[0]['country'].'", 
							"Country" : "'.$record1[0]['country'].'", 
							"country_flag_url" :"http://miprojects.com.php53-16.ord1-1.websitetestlink.com/projects/evendor/public/uploads/users/'.$record1[0]['country_flag'].'",
							"JoinTime"	: "'.$record[0]['added_date'].'"
					...
					if(count($records)>0)
						{
							...
							foreach($records as $key=>$Parray)
							{
								...
								$response['Purchase'][$key]['ProductThumbnail']=$thumbnail_path;
								$response['Purchase'][$key]['Producturl']="http://miprojects.com.php53-16.ord1-1.websitetestlink.com/projects/evendor/api/download/index/apicall/Bookdownload/apikey/".$jsonObj->apikey."/bookid/".$Parray['product_id']."/bookname/".$bookname;
				...
				case "GetStores_old":
					...
					foreach($data_countries as $key=>$Parray)
							{
								...
								$response['Allstores'][$key]['country_flag_url']="http://miprojects.com.php53-16.ord1-1.websitetestlink.com/projects/evendor/public/uploads/users/".$Parray['country_flag'];
							}
					...
					foreach($data_countries1 as $key1=>$Parray)
							...
							$response['Allstores'][$key+1]['country_flag_url']="http://miprojects.com.php53-16.ord1-1.websitetestlink.com/projects/evendor/public/uploads/users/".$Parray['country_flag'];
				...
				case "GetStores":
					...
					foreach($data_countries as $key=>$Parray)
							...
							$response['Allstores'][$key+1]['country_flag_url']="http://miprojects.com.php53-16.ord1-1.websitetestlink.com/projects/evendor/public/uploads/users/".$tempcountry_flag;
...
WHERE replace( country, ' ', '' ) = '".$countrrep."' ";
			...
			//new arrivals
				...
				if($jsonObj->UserId!='')
				{
					...
					if(!empty($jsonObj->StoreId) && $jsonObj->StoreId>0) 
					{
						$sql="SELECT prod.*,c.category_name,g.genre,storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id=storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1'  AND prod.file_name!='' and is_featured=1  AND storeprice.country_id='$jsonObj->StoreId' ORDER BY prod.product_type*1 ASC";
					}
					else
					{
						$sql="SELECT prod.*,c.category_name,g.genre, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id = storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1' AND prod.file_name!='' and is_featured=1   ORDER BY prod.product_type*1 ASC";
				...
				case "GetOnlyBestSeller":
					...
					if(!empty($jsonObj->StoreId) && $jsonObj->StoreId>0) 
					{
						$sql="SELECT prod.*,c.category_name,g.genre,storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id=storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1'  AND prod.file_name!='' AND storeprice.country_id='$jsonObj->StoreId' ORDER BY prod.best_seller DESC";
					}
					else
					{
						$sql="SELECT prod.*,c.category_name,g.genre, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id = storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1' AND prod.file_name!=''  ORDER BY prod.best_seller DESC";
				 	}
				...
				case "GetOnlyNewArrivals":
					...
					$sql="SELECT prod.*,c.category_name,g.genre,storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id=storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1'  AND prod.file_name!='' AND storeprice.country_id='$jsonObj->StoreId' ORDER BY prod.add_time DESC limit 0,90";	
					}
					else
					{
						...
						$sql="SELECT prod.*,c.category_name,g.genre, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id = storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1' AND prod.file_name!=' ORDER BY prod.add_time DESC limit 0,90";
				 	}
				     	...
				     	if(count($records)>0)
						{
							...
							foreach($records as $key=>$Parray)
							{
							  ...
							  // get symbol with price $retVal1[0]['currency_sign'];
								...
								$response['groupBooks'][$key]['Producturl']="http://www.miprojects.com.php53-16.ord1-1.websitetestlink.com/projects/evendor/api/download/index/apicall/Bookdownload/apikey/".$jsonObj->apikey."/bookid/".$Parray['product_id']."/bookname/".$bookname;
							...
							if($rs[1]!='')
							{
								...
								{
								$response['groupBooks'][$key]['rating']=$rs[0]+0.50;	
								}
							}
							else
							{
