           	...
           	switch($jsonObj->apicall)
			{
				...
				case "GetLibrary":
					...
					if(!empty($jsonObj->StoreId) && $jsonObj->StoreId>0) 
					{
						    ...
						    $store_qry='SELECT is_store from  pclive_country where id="'.$jsonObj->StoreId.'" order by id desc LIMIT 1';
							$store_result = $db->query($store_qry);
							$store_record = $store_result->FetchAll();
							$store_info=$store_record[0];	
							if($store_info['is_store']==1)
				...
				case "GetStores":
					$get_all_countries = "select id, country as store from pclive_country where status='1' and is_store=1";
					$res_all_countries = $db->query($get_all_countries);
					$data_countries =  $res_all_countries->FetchAll();
...
WHERE replace( country, ' ', '' ) = '".$countrrep."' ";
							...
							if($result)
							{
								  $response.='{"Message":"Success", "UpdationResponse":"Record Updated Successfully" ,"UpdatedCountry":"'.$data_country[0]['id'].'"}';
