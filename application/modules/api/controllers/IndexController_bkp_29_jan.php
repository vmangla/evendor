				...
				case "UserLogin":
				    if(isset($jsonObj->EmailId) && !empty($jsonObj->EmailId) && isset($jsonObj->Password)) 
					{
						...
						$record1 = $result1->FetchAll();	
						  ...
						  $devicesql = 'SELECT * FROM  pclive_deviceID where user_publisher_id="'.$record[0]['id'].'" and table_name="pclive_users"';
						  $deviceresult = $db->query($devicesql);
						  $devicerecord = $deviceresult->FetchAll();					   
						
						  
						 
						  $devicesqln = 'SELECT * FROM  pclive_deviceID where user_publisher_id="'.$record[0]['id'].'" and table_name="pclive_users" and device_id="'.$jsonObj->DeviceId.'"';          
						  ...
						  {
							   $devicesqlinsert = 'insert into pclive_deviceID set user_publisher_id="'.$record[0]['id'].'", table_name="pclive_users",device_id="'.$jsonObj->DeviceId.'"';
						  ...
						  //can not login with this device id	
						    ...
						    $response = '{
							"Message":"Can not login more than devices",
							"apikey":"",
							"error":"true"
							}';
...
WHERE replace( country, ' ', '' ) = '".$countrrep."' ";
			...
			//new arrivals
				...
				case "GetOnlyNewArrivals":
					...
					$sql="SELECT prod.*,c.category_name,g.genre,storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id=storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id  WHERE prod.admin_approve='1'  AND prod.file_name!='' AND storeprice.country_id='$jsonObj->StoreId' ORDER BY prod.add_time DESC limit 0,90";	
					}
					else
					{
						...
						$sql="SELECT prod.*,c.category_name,g.genre, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id = storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id WHERE prod.admin_approve='1' AND prod.file_name!=' ORDER BY prod.add_time DESC limit 0,90";
				 	}
