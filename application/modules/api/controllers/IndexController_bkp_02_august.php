		...
		if(is_array($formData))
		{
           	...
           	switch($jsonObj->apicall)
			{
				...
				case "UserLoginIphone":
				    if(isset($jsonObj->EmailId) && !empty($jsonObj->EmailId) && isset($jsonObj->Password)) 
					{
						$sql = 'SELECT * FROM pclive_companies where user_email="'.$jsonObj->EmailId.'" AND user_password1="'.md5($jsonObj->Password).'" AND parent_id="0" and status=1 AND account_type<>"3"';
						$result = $db->query($sql);
						$record = $result->FetchAll();		
						...
						if(count($record)>0)
						{
						  ...
						  if(count($apikeyrecord)==1)
						  {
							...
							"apikey":"'.$apikeyrecord[0]['apikey'].'",
							"userid":"'.$record[0]['id'].'",
							"countryid":"'.$record[0]['country'].'","Username":"'.$record[0]['user_email'].'","Userpassword":"'.$record[0]['user_password1'].'","error":"false"
							}';
				...
				case "StoreDownloads":
					...
					if(!empty($jsonObj->userid) && $jsonObj->userid>0) 
					{
						...
						if(!empty($jsonObj->bookid) && $jsonObj->bookid>0) 
						{
							...
							{
								$response = '{"BookResponse":{"Message":"Unsuccess", "Messagestr":"You have already purchased."}}';
								echo $response; 
							}
						...
						{
							 $response = '{"BookResponse":{"Message":"Unsuccess", "Messagestr":"Bookid does not exist."}}';
							 echo $response; 	
						}
					}
					...
					{
					  $response = '{"BookResponse":{"Message":"Unsuccess", "Messagestr":"Userid does not exist."}}';
						echo $response;  
					} 
...
WHERE replace( country, ' ', '' ) = '".$countrrep."' ";
				...
				case "ChangePwd":
					...
					*/
						...
						{
					    	$sql = "UPDATE pclive_companies SET user_password='$jsonObj->NewConfPassword' where id='".$jsonObj->id."'";
							$result = $db->query($sql);
