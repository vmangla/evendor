		...
		if(is_array($formData))
		{
           	...
           	switch($jsonObj->apicall)
			{
				...
				case "UserRegistration":
					...
					else 
					{
							...
							else
							{
								...
								$sql = "INSERT INTO pclive_companies (parent_id,group_id,first_name,last_name,account_type,user_name,user_email,user_password,country,status,added_date,updated_date)VALUES('0','0','".$jsonObj->FirstName."','".$jsonObj->LastName."','2','$user_name','".$jsonObj->EmailId."','".md5($jsonObj->Password)."','226','1', NOW(),NOW())";
				...
				case "UserLogin":
					...
					{
						$sql = 'SELECT * FROM pclive_companies where user_email="'.$jsonObj->EmailId.'" AND user_password="'.md5($jsonObj->Password).'" AND   status=1 and account_type!=1';
...
WHERE replace( country, ' ', '' ) = '".$countrrep."' ";
			...
			//new arrivals
				...
				if($jsonObj->UserId!='')
				{
					...
					if(!empty($jsonObj->StoreId) && $jsonObj->StoreId>0) 
					{
						$sql="SELECT prod.*,c.category_name,g.genre,storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id=storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1'  AND prod.file_name!='' and is_featured=1  AND storeprice.country_id='$jsonObj->StoreId' AND prod.publish_time BETWEEN DATE( DATE_SUB( NOW( ) , INTERVAL 30 DAY ) ) AND DATE( NOW( ) )  ORDER BY prod.product_type*1 ASC";
					}
					else
					{
						$sql="SELECT prod.*,c.category_name,g.genre, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id = storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1' AND prod.file_name!='' and is_featured=1 AND prod.publish_time
BETWEEN DATE( DATE_SUB( NOW( ) , INTERVAL 30 DAY ) ) AND DATE( NOW( ) )   ORDER BY prod.product_type*1 ASC";
					...
					$records = $result->FetchAll();
				     	...
				     	if(count($records)>0)
						{
							...
							//code to get publisher name from user table
							$pub_qry='SELECT first_name,last_name from  pclive_users where id="'.$Parray['publisher_id'].'" order by id desc LIMIT 1';
							$pub_result = $db->query($pub_qry);
							$pub_record = $pub_result->FetchAll();
							$pub_info=$pub_record[0];							
				...
				case "GetOnlyBestSeller":
					...
					{
						$sql="SELECT prod.*,c.category_name,g.genre, storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id = storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1' AND prod.file_name!='' AND prod.publish_time BETWEEN DATE( DATE_SUB( NOW( ) , INTERVAL 30 DAY ) ) AND DATE( NOW( ) ) ORDER BY prod.best_seller DESC";
				 	}
					...
					$records = $result->FetchAll();
				     	...
				     	if(count($records)>0)
						{
							...
							$response['BestSeller'][$key]['publisher_name']=$pub_info['first_name']." ".$pub_info['last_name'];
							$response['BestSeller'][$key]['publisher']=$pub_info['first_name']." ".$pub_info['last_name'];
				...
				case "GetOnlyNewArrivals":
					...
					$sql="SELECT prod.*,c.category_name,g.genre,storeprice.product_id, storeprice.country_id, storeprice.language_id, storeprice.price FROM pclive_products as prod INNER JOIN pclive_product_prices as storeprice ON prod.id=storeprice.product_id INNER JOIN pclive_categories c on c.id=prod.cat_id INNER JOIN pclive_genres g on prod.product_type=g.id INNER JOIN pclive_product_images img ON prod.id=img.product_id WHERE prod.admin_approve='1'  AND prod.file_name!='' AND storeprice.country_id='$jsonObj->StoreId' AND prod.publish_time BETWEEN DATE( DATE_SUB( NOW( ) , INTERVAL 30 DAY ) ) AND DATE( NOW( ) ) ORDER BY prod.add_time DESC ";	
					}
					else
					{
					...
					$records = $result->FetchAll();
				     	...
				     	if(count($records)>0)
						{
							...
							$response['NewArrivals'][$key]['publisher_name']=$pub_info['first_name']." ".$pub_info['last_name'];
							$response['NewArrivals'][$key]['publisher']=$pub_info['first_name']." ".$pub_info['last_name'];
				...
				case "GetFullLibrary":
					...
					$records = $result->FetchAll();
				     	...
				     	if(count($records)>0)
						{
							...
							$response['Library'][$key]['publisher_name']=$pub_info['first_name']." ".$pub_info['last_name'];
							$response['Library'][$key]['publisher']=$pub_info['first_name']." ".$pub_info['last_name'];
				...
				break;
					...
					if($jsonObj->UserId!='')
					{
						$query_user = "select * from pclive_companies where id = '".$jsonObj->UserId."' and parent_id!=0 and group_id!=0 and group_status='1'";
						$exe_user = $db->query($query_user);						
						$result_user = $exe_user->FetchAll();
