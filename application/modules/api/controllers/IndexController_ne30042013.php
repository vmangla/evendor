           	...
           	switch($jsonObj->apicall)
			{
				...
				case "BookDownload":
					...
					{	
						...
						$sql = "select * from pclive_products where id='$jsonObj->bookid'";
						$result = $db->query($sql);
						$records = $result->FetchAll();
						...
						//readfile($this->view->serverUrl().$this->view->baseUrl()."/public/uploads/epubfile/".$filename);
						exit();
		 
					}
					else 
					{
					$response = '[{"Error":{"Message":"This Book does not exist"}}]';
					echo $response;  
					}
