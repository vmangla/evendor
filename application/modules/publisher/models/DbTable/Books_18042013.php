...
class Publisher_Model_DbTable_Books extends Zend_Db_Table
{
	...
	public function isPriceExist($productID,$countryID,$languageID)
	{
		$retVal=false;
	...
	public function getAllStores()
	{
		...
		$store_query="SELECT GROUP_CONCAT(country,'%^') as country_names, GROUP_CONCAT(id,'%^') as country_ids, continent_id FROM ".$this->_countryname." WHERE id<>0 GROUP BY continent_id ORDER BY continent_id DESC";
	...
	public function getPublicationIdsByCategoryName($storeId,$cat_Name,$admin_Approve=1,$is_Featured=0,$parentBrandID=0)
	{
		$list=array();
		...
		{
			$query->where('prod.parent_brand_id= ?', $parentBrandID);
		}	
