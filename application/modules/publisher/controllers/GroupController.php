...
class Publisher_GroupController extends Zend_Controller_Action
{
	...
	public function indexAction()
	{
		...
		{	
			...
			$groupList=$this->modelPublisher->getGroupListSearch($this->parentPublisherId,$searchword);
		}
		else
		{
			...
			*/
			...
			if(count($formErrors)==0)
			{
				...
				if($this->getRequest()->isPost())
				{
					...
					$implode_module_active = @implode(",",$formData['module_activ']);
					...
					$groupUserData=array(
								   ...
								   'last_name'=>$formData['last_name'],
									...
									'module_active'=>$implode_module_active,
									'profile_status'=>'1',
						...
						if($lastId>0)
						{
							...
							{
								$dataArray=array("publisher_id"=>$formData['publisher_author_id'],
												"product_id"=>$publications_id,
												"group_user_id"=>$lastId);
						...
						if($lastId>0)
						{
							...
							if($mail->send())
							{
							$this->_flashMessenger->addMessage('<div class="div-success">Group user created successfully</div>');
							} 
							else 
							{	
			...
			if($this->getRequest()->isPost() && isset($_REQUEST['edit_group']) && $_REQUEST['edit_group']=='Edit Group')
			{
				...
				if(count($formErrors)==0)
				{
					...
					if(count($publication_active)>0)
					{
						foreach($publication_active as $publications_id)
						{
							$dataArray=array("publisher_id"=>$publishr_user['parent_id'],
											"product_id"=>$publications_id,
											"group_user_id"=>$id);
							$gg=$this->modelPublisher->insertAssignUserPublication($dataArray);
						}
