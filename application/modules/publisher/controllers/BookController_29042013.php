...
exit;	
	...
	public function createAction()
	{
		...
		if($this->getRequest()->isPost() && (!empty($formData['createbook']) && $formData['createbook']=='Add Publication'))
		{
	
			if(!empty($formData['createbook']) && $formData['createbook']=='Add Publication')
			{ 
					...
					elseif(!CommonFunctions::is_isbn_13_valid($formData['isbn_number']))
					{
					...
					if($this->modelBooks->isExist('title="'.$formData['title'].'"'))
					{
					...
					}elseif(isset($formData['issn_number']) && !empty($formData['issn_number']))
					{
						if(!(CommonFunctions::is_issn_valid($formData['issn_number'])))
						{
					...
					}elseif(isset($formData['issn_number']) && !empty($formData['issn_number']) &&  $this->modelBooks->isExist("isbn_number='$formData[issn_number]'"))
					{
						if(!(array_key_exists('issn_number',$formErrors)))
						{
					...
					if(count($formErrors)==0)
					{
								...
								$formData['isbn_number']=!empty($formData['issn_number'])?$formData['issn_number']:!empty($formData['isbn_number'])?$formData['isbn_number']:"";
	...
	public function deleteAction()
    {
		...
		if($id>0 && $this->modelBooks->isExist($id))
		{
			...
			if($price_success && $image_success && $success)
			{
    ...
    {		
		...
		if($this->getRequest()->isPost())
		{
			...
			if(isset($post_data['cat_id']) && !empty($post_data['cat_id']))
			{
				...
				{
					$titleBrandDropdown=$this->modelBooks->getTitleBrandList($title_brand,$this->parentPublisherId);
				...
				elseif(strtolower(trim($cat_info['category_name']))==strtolower(trim('Magazine')) || strtolower(trim($cat_info['category_name']))==strtolower(trim('Magazines')))
				{
					...
					$isbn_field='<input type="text" name="issn_number" id="issn_number" value=""  class="req issn"  message="Please enter issn number 2" invalidmessage="Please enter a valid ISSN number format." /><br /><br />';
