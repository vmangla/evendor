...
?>
	...
	public function updateproductsubscriptionAction()
	{
...
if(isset($_REQUEST))
		{
					...
					$this->modelBooks = new Publisher_Model_DbTable_Subscriptions();
					
					$check_data = $this->modelBooks->fetchAll('country_sub="'.$_REQUEST['country'].'" and language_sub="'.$_REQUEST['languageval'].'" and subscription_type="'.$_REQUEST['subscriptionval'].'" and id!="'.$_REQUEST['priceid'].'" and product_id="'.$_REQUEST['productid'].'"');
					...
					if(!(isset($_REQUEST['subscriptionval'])) || trim($_REQUEST['subscriptionval'])=="")
					{
						$formErrors['subscription_type']="Please select subscription type";
					}
					if(!(isset($_REQUEST['numberofissuesval'])) || trim($_REQUEST['numberofissuesval'])=="")
					{
						$formErrors['number_of_issues']="Please enter number of issues";
					}
					if(!(isset($_REQUEST['priceval'])) || trim($_REQUEST['priceval'])=="")
					{
						$formErrors['individual_price']="Please enter individual price";
					}
					...
					if(count($formErrors)==0)
					{
						...
						{*/
							$priceData=array(
									'country_sub'=>$_REQUEST['country'],
									'language_sub'=>$_REQUEST['languageval'],
									'subscription_type'=>$_REQUEST['subscriptionval'],
									'number_of_issues'=>$_REQUEST['numberofissuesval'],
									'individual_price'=>$_REQUEST['priceval'],
									'group_price_sub'=>$_REQUEST['grouppriceval']
									 );
 ...
 <?php	
		...
		{
		?>
		<a class="action-icon" href="<?php echo $this->view->url(array('module'=>'publisher','controller'=>'book','action'=>'inactivesubscription','id'=>$subscriptionList[$jj]['id']),'',true);?>" title="Deactivate Subscription" onclick="return inactivesubscription(this.href,'tab2','book');"><img alt="Active" src="<?php echo $this->view->serverUrl().$this->view->baseUrl();?>/public/css/admin/images/active.png"/></a>	
		<?php  
		}
		else
		{
		?>
		<a class="action-icon" href="<?php echo $this->view->url(array('module'=>'publisher','controller'=>'book','action'=>'activesubscription','id'=>$subscriptionList[$jj]['id']),'',true);?>" title="Activate Subscription" onclick="return activesubscription(this.href,'tab2','book');"><img alt="Inactive" src="<?php echo $this->view->serverUrl().$this->view->baseUrl();?>/public/css/admin/images/inactive.png"/></a>
		<?php	
		}
		?>
		<a href="javascript:void(0);" onclick="return updatesubscription1('<?php echo $subscriptionList[$jj]['id'];?>','<?php echo $this->view->serverUrl().$this->view->baseUrl()."/publisher/book/savesubscription";?>','<?php echo $formData['productid'];?>','<?php echo $subscriptionList[$jj]['id'];?>','<?php echo $this->view->serverUrl().$this->view->baseUrl()."/publisher/book/savesubscription";?>','<?php echo $formData['productid'];?>');">
	...
	?>	
	  <form name="form-add-subscription" id="form-add-subscription" method="post" onsubmit="return updatesubscription2('<?php echo $_POST['priceid'];?>','http://miprojects2.com.php53-6.ord1-1.websitetestlink.com/projects/evendor/publisher/book/updateproductsubscription/productid/<?php echo $_POST['productid'];?>','<?php echo $_POST['productid'];?>');">
	  <fieldset class="dashboard_fieldset">
			<legend>Publication Store & Langauge</legend>
	  		<table width="99%"  class="table-list publisher"  border="0" cellspacing="0" cellpadding="0">
				<tr>
				...
				$countryList   = $modelCountry->getCountryList();
				?>
				<select name="country_sub" id="country_sub" class="req"  message="Please select country">
				<option value="">Select Store</option>
				...
				{
				?>
				<option value="<?php echo $countryList[$ii]['id']; ?>" <?php if($pricedata[0]["country_sub"]==$countryList[$ii]["id"]){ ?> selected <?php } ?> ><?php echo $countryList[$ii]['country']; ?></option>
				<?php
				}
				?>		   
				</select>
				...
				{
				?>
				<option value="<?php echo $languageList[$ii]['id']; ?>" <?php if($pricedata[0]["language_sub"]==$languageList[$ii]['id']){ ?> selected <?php } ?>><?php echo $languageList[$ii]['language_name']; ?></option>
				<?php
				}
				?>		   
				</select>
				...
				<tr>
				<td class="tdleftBold">Subscription type<span class="required">*</span> : </td>
				<td>
				<select name="subscription_type" id="subscription_type"  class="req"  message="Please select subscription type">
				<option value="">Select</option>
				<option value="Weekly" <?php if($pricedata[0]["subscription_type"]=='Weekly') { ?>selected<?php } ?>>Weekly</option>
				<option value="Monthly" <?php if($pricedata[0]["subscription_type"]=='Monthly') { ?>selected<?php } ?>>Monthly</option>
				<option value="Quarterly" <?php if($pricedata[0]["subscription_type"]=='Quarterly') { ?>selected<?php } ?>>Quarterly</option>
				<option value="Half Yearly" <?php if($pricedata[0]["subscription_type"]=='Half Yearly') { ?>selected<?php } ?>>Half Yearly</option>
				<option value="Yearly" <?php if($pricedata[0]["subscription_type"]=='Yearly') { ?>selected<?php } ?>>Yearly</option>
				</select>
				<br/>
				<span id="err_subscription" style="color:#f00;"></span>
				</td>
				...
				<td class="tdleftBold">Number of issues<span class="required">*</span> : </td>
				<td>
				<input type="text" name="number_of_issues" id="number_of_issues"  class="req" value="<?php echo $pricedata[0]["number_of_issues"];?>"  message="Please enter number of issues"/>
				<?php if(array_key_exists('number_of_issues',$formErrors)){echo '<div class="inline-error">'.$formErrors['number_of_issues'].'</div>';}?>
				<br/>
				<span id="err_numberofissues" style="color:#f00;"></span>
				</td>
				...
				<td class="tdleftBold">Individual Price<span class="required">*</span> : </td>
				<td>
				<input type="text" name="individual_price" id="individual_price"  class="req" value="<?php echo $pricedata[0]["individual_price"];?>"  message="Please enter price"/>
				<?php if(array_key_exists('individual_price',$formErrors)){echo '<div class="inline-error">'.$formErrors['individual_price'].'</div>';}?>
				<br/>
				<span id="err_individualprice" style="color:#f00;"></span>
				</td>
				...
				<td class="tdleftBold">Group Price : </td>
				<td>
				<input type="text" name="group_price_sub" id="group_price_sub" value="<?php echo $pricedata[0]["group_price_sub"];?>" />
				<?php if(array_key_exists('group_price_sub',$formErrors)){echo '<div class="inline-error">'.$formErrors['group_price_sub'].'</div>';}?>
				</td>
				</tr>
		<tr>
				...
				<tr>
					<td colspan="2">
						<div style="text-align:center;">
							<input type="hidden" name="createsubscription" value="Update 
							Subscription">
							<input type="hidden" name="productid" id="productid" value="<?php echo $_POST['productid']; ?>" />
							<input type="hidden" name="priceid" id="priceid" value="<?php echo $_POST['priceid']; ?>" />
							<input type="submit" name="updatesubscription" id="updatesubscription" value="Update Subscription" class="button-Save"/>
						</div>
...
echo "</table></fieldset>";
				...
				{ 
						...
						if(count($formErrors)==0)
						{
									...
									if($lastId>0)
									{
										...
										$adminRecord  =$adminDetails->CurrentRow('8');
		...
		public function addsubscriptionAction()
		{
			...
			if($this->getRequest()->isPost() && (!empty($formData['createsubscription']) && $formData['createsubscription']=='Add Subscription'))
			{
						...
						if(count($formErrors)==0)
						{
							...
							$priceData=array(
									'product_id'=>$formData['productid'],
									'country_sub'=>$formData['country_sub'],
									'language_sub'=>$formData['language'],
									'subscription_type'=>$formData['subscription_type'],
									'number_of_issues'=>$formData['number_of_issues'],
									'individual_price'=>$formData['individual_price'],
									'group_price_sub'=>$group_price
									 );
	...
	public function deletesubscriptionAction()
    {
		...
		{
			$success=$modelSubscription->delete('id='.$id);
		...
		<a href="javascript:void(0);" onclick="return updatesubscription1('<?php echo $priceList[$jj]['id'];?>','<?php echo $this->view->serverUrl().$this->view->baseUrl()."/publisher/book/savesubscription";?>','<?php echo $_POST['productid'];?>','<?php echo $priceList[$jj]['id'];?>','<?php echo $this->view->serverUrl().$this->view->baseUrl()."/publisher/book/savesubscription";?>','<?php echo $_POST['productid'];?>');">
		<img src="<?php echo $this->view->serverUrl().$this->view->baseUrl()."/public/css/publisher/images/icon_edit.png"?>" width="10" height="10"/>
		</a>
		<?php	
		echo "</div></td></tr>";
...
echo "</table>";
    ...
    {		
		...
		$isbn_numb=$this->_request->getParam('isbn',0);
		if($isbn_numb == 0)
		{
			$isbn_numb ='';
		}
				...
				$titleBrandDropdown=$this->modelBooks->getTitleBrandList('',$this->parentPublisherId,$post_data['cat_id']);
				 ...
				 //echo "Count brand : - ".$titleBrandDropdown."<br>";
					...
					if(strtolower(trim($cat_info['category_name']))==strtolower(trim('Magazine')) || strtolower(trim($cat_info['category_name']))==strtolower(trim('Magazines')) || strtolower(trim($cat_info['category_name']))==strtolower(trim('Newspapers')))
					{
						...
						$isbn_label='ISSN Number: ';
						$isbn_field='<input type="text" name="isbn_number" id="isbn_number" value="'.$isbn_numb.'"  class="isbn"  message="Please enter ISSN number" invalidmessage="Please enter a valid ISSN number format." maxlength="25" /><br /><br />';
						$isbn_field='<input type="text" name="isbn_number" id="isbn_number" value="'.$isbn_numb.'"  class="isbn"  message="Please enter ISSN number" invalidmessage="Please enter a valid ISSN number format." maxlength="25"/><br /><br />';
					}
					else if(strtolower(trim($cat_info['category_name']))==strtolower(trim('Ebook')) || strtolower(trim($cat_info['category_name']))==strtolower(trim('Ebooks')))
					{
						...
						$isbn_label='ISBN Number:';
						$isbn_field='<input type="text" name="isbn_number" id="isbn_number" value="'.$isbn_numb.'"  maxlength="25"/><br /><br />';
