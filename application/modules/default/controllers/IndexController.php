...
class IndexController extends Zend_Controller_Action
{
	...
	public function init()
    {
		...
		$query = "SELECT DISTINCT (prod.product_type), gen.genre FROM pclive_products prod
				  INNER JOIN pclive_genres gen ON prod.product_type = gen.id INNER JOIN pclive_product_images as prImg ON prImg.product_id = prod.id  INNER JOIN pclive_product_prices as store_price ON store_price.product_id =prod.id and store_price.country_id	='".$storeSession->sid."'
				  WHERE prod.cat_id =1 and prod.admin_approve='1' and gen.status=1  ORDER BY gen.genre ASC ";		
		$result = $db->query($query); 		
		$getnewspapers_genres = $result->fetchAll();	
		
		//$getnewspapers_genres = $getnewspapers_genres->toArray();
		//echo "<pre>";
		//print_r($getnewspapers_genres);
		//exit;
		...
		*/
		...
		$query_mag = "SELECT DISTINCT (prod.product_type), gen.genre FROM pclive_products prod
				  INNER JOIN pclive_genres gen ON prod.product_type = gen.id INNER JOIN pclive_product_images as prImg ON prImg.product_id = prod.id INNER JOIN pclive_product_prices as store_price ON store_price.product_id =prod.id and store_price.country_id	='".$storeSession->sid."'
				  WHERE prod.cat_id =2 and prod.admin_approve='1'  and gen.status=1 ORDER BY gen.genre ASC ";		
		$result_mag = $db->query($query_mag); 		
		$getmagazine_genres = $result_mag->fetchAll();		
			//echo "<pre>";
			//print_r($getmagazine_genres);
			//exit;
		...
		$query_ebook = "SELECT DISTINCT (prod.product_type), gen.genre FROM pclive_products prod
				  INNER JOIN pclive_genres gen ON prod.product_type = gen.id INNER JOIN pclive_product_images as prImg ON prImg.product_id = prod.id  INNER JOIN pclive_product_prices as store_price ON store_price.product_id =prod.id and store_price.country_id	='".$storeSession->sid."'
				  WHERE prod.cat_id =3 and prod.admin_approve='1'  and gen.status=1 ORDER BY gen.genre ASC ";		
	...
	/*	$approvedBookList=$this->modelBooks->getPublicationIdsByCategoryName($storeId,'eBook','1','0');
		...
		$approvedAndFeaturedBookList=$this->modelBooks->getPublicationFeaturedItemFront($storeId,'3','1','1');
		$this->view->approvedAndFeaturedBookList=$approvedAndFeaturedBookList;
		...
		$approvedMagazineIssuesList=$this->modelBooks->getPublicationFeaturedItemFront($storeId,'2','1','1');
		$this->view->approvedMagazineList=$approvedMagazineIssuesList;

		/*******************************************************************/
		
		/*************************  Newspaper Details  ***************************************/
		$approvedNewLetterIssuesList=$this->modelBooks->getPublicationFeaturedItemFront($storeId,'1','1','1');
		...
		$query_mostcomment = "SELECT prod.title,prod.id as product_id,count(prod.id) as tot,rvw.*,pri.* FROM pclive_products prod
				  INNER JOIN  pclive_review rvw ON prod.id = rvw.productid INNER JOIN  pclive_product_prices pri ON prod.id = pri.product_id 
