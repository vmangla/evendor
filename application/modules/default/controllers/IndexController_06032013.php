...
class IndexController extends Zend_Controller_Action
{
	...
	public function init()
    {
		...
		//newspaper genres
		$getnewspapers_genres = $this->modelBooks->fetchAll(
		$this->modelBooks->select()
		->from($this->modelBooks, array('DISTINCT(product_type)'))
		->where('cat_id = ?', 1)
		);
		...
		//magazine genres
		$getmagazine_genres = $this->modelBooks->fetchAll(
		$this->modelBooks->select()
		->from($this->modelBooks, array('DISTINCT(product_type)'))
		->where('cat_id = ?', 2)
		);
		$getmagazine_genres = $getmagazine_genres->toArray();
		
		 
		$this->view->getmagazine_genres = $getmagazine_genres;
		
		//ebook genres
		$getebooks_genres = $this->modelBooks->fetchAll(
		$this->modelBooks->select()
		->from($this->modelBooks, array('DISTINCT(product_type)'))
		->where('cat_id = ?', 3)
		);
		$getebooks_genres = $getebooks_genres->toArray();
