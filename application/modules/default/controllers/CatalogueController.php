...
class CatalogueController extends Zend_Controller_Action
{
    ...
    public function init()
    {
		...
		foreach($categoriesList as $key=>$catArray)
		{
			...
			$records_v = $this->modelBooks->getPublicationIdsByCategoryNameCountFront($storeSession->sid,$catArray['category_name'],'1','0');
			$categoriesList[$key]['total_publications']=count($records_v);
		}
	...
	public function indexAction()
    {
		...
		else
		{
			...
			{
			   $approvedPublicatiobList=$this->modelBooks->getBackIssueFront($store_id,$backissue);
			}
			else if($allebooks!="" && $store_id!="" && $language_id!="")
			{
			   $approvedPublicatiobList=$this->modelBooks->getBackIssueFront($store_id,$allebooks);
			}
			else
			{
			   $approvedPublicatiobList=$this->modelBooks->getPublicationIdsByCategoryNameFront($storeId,'','1','0');
			}
		}
    ...
    {			
		...
		if($id>0 && $this->modelBooks->isPriceExist($id,$store_id,$language_id))
		{
			...
			$storePublicationDetail=$this->modelBooks->getStorePublicationDetailFront($id,$store_id,$language_id);
			$this->view->storePublicationDetail=$storePublicationDetail;
			
			
			#######################  best seller book #################################
			$bestsellerBook = $this->modelBooks->getBestSellerProductFront($store_id,'3','1','1');
			...
			$reviewsData = $this->commentObj->fetchAll("productid='".$id."' and comments!=''","id desc");
			...
			if(!empty($publicationInfo) && count($publicationInfo)>0)
			{
				...
				if(!empty($catInfo) && count($catInfo)>0)
				{
					if($publicationInfo['parent_brand_id']>0)
					{
						...
						$backIssuesList=$this->modelBooks->getBackIssueFront($store_id,$publicationInfo['parent_brand_id']);
						...
						$similarPublicationList=$this->modelBooks->getPublicationIdsByCategoryNameFront($store_id,$cateDet['category_name'],'1','0',0,$id);
					...
					else
					{
						...
						$backIssuesList="";
						$similarPublicationList=$this->modelBooks->getPublicationIdsByCategoryNameFront($store_id,$catInfo['category_name'],'1','0',$publicationInfo['parent_brand_id'],$id);
					}
				...
				$this->view->category_name = $catInfo['category_name'];
				}
			}
	...
	public function insertusercommentAction()
	{
		...
		$ratting_val = $postData['ratting_value'];
		...
		{
			$commentData = $this->commentObj->fetchAll("userid='".$user_id."' and productid ='".$product_id."'","id desc");
			if(count($commentData)>0)
			{
				...
				$where = "userid='".$user_id."' and productid='".$product_id."' and id='".$commentData[0]['id']."'";
				$this->commentObj->update($data,$where);
			}
			else
			{
				$current_date = date("Y-m-d H:i:s");
